...
- diagnostics：`P` 调用 `opencode --version`、`db path`、`providers list`、`models --verbose`、`agent list`、`mcp list`，保存到 `~/.pad/opencode-diagnostics/`。
...
- `opencode serve`：适合长期/headless server；需要 server 生命周期、端口、认证和关闭策略后再接。`web` 已作为一次性浏览器入口接入，`attach <url>` 已支持从剪贴板发起。
- `opencode run`：适合非交互自动化；和 PAD 当前 pane/TUI 工作流不同，暂不混入普通快捷键。
- `opencode plugin` 以及 mcp/agent 的增删改认证命令：属于 OpenCode 配置管理，未来更适合放进设置页。`agent list`、`mcp list`、`providers list` 和 `models --verbose` 已作为只读诊断导出。
