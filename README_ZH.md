...
| `A` / `U` | 归档 / 恢复选中 session |
| `Z` | 切换归档 session 视图 |
| `E` / `S` / `I` / `L` / `G` / `X` / `B` / `O` / `P` / `Y` / `W` | 导出/导入 OpenCode JSON、安装 plugin、打开 PR、运行剪贴板 prompt、启动本地 server、stats、诊断、attach server URL，或打开 OpenCode Web |
| `r` | 刷新 |
| `Ctrl+F` | 搜索 panel |
| `/` | 打开设置 |
| `F1` | 设置 |
