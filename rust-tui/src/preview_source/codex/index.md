...
- `parser.rs` / `parser/`：Codex JSONL transcript 解析，用借用字段减少字符串复制，并只保留最近预览 turn。
...
- `subagent.rs`：压缩 Codex subagent notification，合并到主 turn 预览。
- `tests.rs`：Codex transcript、图片、上下文过滤和 status probe 单元测试。
