...
- `../event.rs` / `key_pipeline.rs`: event/key routing by mode.
...
- `mouse.rs` / `mouse_pipeline.rs` / `mouse_pipeline_tests.rs`: mouse dispatch facade、hit testing、click/selection/hover，以及 child mouse-reporting 与 PAD terminal scrollback 仲裁和回归测试。
- `../event.rs` 内联 `loop_core` / `../event.rs` 内联 `loop_state`: main event loop state.
- `refresh_pipeline.rs`：周期刷新、异步结果与 hook drain；每帧轮询 terminal controller，并用共享 split placement 批量 resize 当前 tab 的可见 panes。
