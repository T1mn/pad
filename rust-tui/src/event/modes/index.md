...
- `agent_launcher.rs` / `agent_launcher_tests.rs`: launch selected agents in native PTY tabs, retain explicit tmux compatibility routing, and apply Codex/Claude runtime wiring.
- `agent_style.rs`: agent style selector key handling.
- `delete_confirm.rs` / `help.rs` / `telegram.rs`: delete confirmation, help, and Telegram modal keys.
