...
pub fn detect_state(raw_content: &str) -> super::model::AgentState {
    ...
    let tail: String = lines.iter().rev().take(5).copied().collect::<Vec<&str>>().join("\n");
    ...
    let busy_keywords = [
        "thinking", "processing", "generating", "analyzing", "compiling",
        "searching", "reading", "writing", "editing", "running",
        "ctrl+c to interrupt", "esc to interrupt",
        "ctrl-c to interrupt", "esc to stop",
    ];
    ...
    let waiting_patterns = [
        "yes, allow once", "yes, allow always", "allow once",
        "continue?", "[y/n]", "[yes/no]",
    ...
    let last_line = lines.iter().rev().find(|l| !l.trim().is_empty()).map(|l| l.trim());
