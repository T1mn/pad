...
- `history_claude.rs` / `history_codex.rs` / `history_codex/` / `history_gemini.rs` / `history_grok.rs` / `history_opencode.rs`：历史会话构建；Codex 拆分 merge、entry 与 session cache snapshot。
