...
- `../build.rs` 内联 `finalize` / `../build.rs` 内联 `logging`：folder 排序收尾与构建耗时日志。
- `history_claude.rs` / `history_codex.rs` / `history_codex/` / `history_gemini.rs` / `../build.rs` 内联 `history_grok` / `history_opencode.rs`：历史会话构建；Codex 拆分 merge、entry 与 session cache snapshot。
