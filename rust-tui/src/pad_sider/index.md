...
- `mod.rs` 内联 `cli`：解析 `toggle` / `ui` 命令。
- `tmux.rs` / `tmux/` / `tmux_args.rs`：F10 全屏 overlay 辅助栏、隐藏、恢复、tmux 参数日志格式与 zoom 状态处理。
...
- `mod.rs` 内联 `ignore`：tree、文件搜索与 index map 共用的忽略目录规则。
...
- `mod.rs` 内联 `preview`：右侧文件预览模型，区分 Markdown、文本、diff、目录与缺失文件。
- `mod.rs` 内联 `preview_cache`：右侧预览按路径、mtime 和大小缓存文件内容，记录慢加载日志。
- `preview_render_cache.rs`：缓存右侧预览已渲染行，滚动时只取可见窗口及缓存失效测试。
- `sizing.rs`：sider 45%-65% 小步宽度档位与 tmux resize。
- `fs.rs`：文件统计、相对路径与完整文本读取。
- `tests.rs`：集中覆盖根级文件读取、索引、宽度、tmux 参数、Codex diff 与预览缓存回归。
