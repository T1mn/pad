mod options {
    use super::super::options::should_restore_target_zoom;
...
mod pane {
    use super::super::{pane::parse_pane_info, PANE_INFO_SEP};
    ...
    fn pane_info_parser_rejects_wrong_field_count() {
        let missing = format!(
            "%7{PANE_INFO_SEP}@1{PANE_INFO_SEP}@2{PANE_INFO_SEP}codex{PANE_INFO_SEP}/tmp/repo"
        );
        let extra = format!(
