...
- `file_preview.rs` / `file_preview/` / `file_preview_tests.rs` / `render_window.rs` / `render_window_tests.rs`：右侧文件/diff/代码预览渲染缓存与可见行窗口。
- `diff.rs` / `diff/`：Codex 单轮 patch 的结构化 diff 解析与 side-by-side / unified 配色渲染。
- `split.rs` / `split_tests.rs`：固定左侧可读宽度，把额外宽度优先留给 preview。
...
- `syntax/`：VS Code Dark+ 风格多语言代码高亮，由 `mod.rs` 聚合。
- `mod.rs` 内联 `file_icons`：文件树语言短标签与强调色。
