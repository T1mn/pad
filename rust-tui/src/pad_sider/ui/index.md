...
- `file_preview.rs` / `file_preview/` / `render_window.rs`：右侧文件/diff/代码预览渲染缓存与可见行窗口。
...
- `markdown.rs` / `markdown/`：紧凑 Markdown 渲染与样式模块。
- `tests.rs`：集中覆盖行号、导航窗口、可见行裁剪、分栏、缩放、文件预览和 Markdown 渲染。
