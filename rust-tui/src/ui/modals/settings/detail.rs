...
pub(super) fn draw_settings_detail_panel(f: &mut Frame, app: &App, area: Rect) {
    ...
    match kind {
        ...
        SettingsDetailKind::ProfilePermissionMode => draw_simple_detail(
            f,
            app,
            area,
            t(app.locale, "settings.profile_permission_mode"),
            simple_value_line(app, kind),
            vec![detail_body_line(app.locale, kind)],
            "Enter/Space cycle · Esc back",
        ),
        SettingsDetailKind::ProfileFullAccess => draw_simple_detail(
            f,
            app,
            area,
            t(app.locale, "settings.profile_full_access"),
            simple_value_line(app, kind),
            vec![detail_body_line(app.locale, kind)],
            "Enter/Space toggle · Esc back",
        ),
        SettingsDetailKind::ProfileUnattended => draw_simple_detail(
            f,
            app,
            area,
            t(app.locale, "settings.profile_unattended"),
            simple_value_line(app, kind),
            vec![detail_body_line(app.locale, kind)],
            "Enter/Space toggle · Esc back",
        ),
