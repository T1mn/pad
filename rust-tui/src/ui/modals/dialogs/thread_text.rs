mod action {
    ...
    pub(in crate::ui::modals::dialogs) fn thread_action_modal_confirm(
        ...
        match (is_cjk_locale(locale), kind, agent_type) {
            (true, ThreadActionKind::Archive, AgentType::Codex) => {
                "确认归档这个 Codex 会话？".into()
            }
            (true, ThreadActionKind::Unarchive, AgentType::Codex) => {
                "确认恢复这个 Codex 会话？".into()
            }
            (true, ThreadActionKind::Restore, AgentType::Codex) => {
                "确认从回收站恢复这个 Codex 会话？".into()
            }
            (true, ThreadActionKind::Archive, AgentType::Claude) => {
                "确认归档这个 Claude 会话？".into()
            }
            ...
            (false, ThreadActionKind::Archive, AgentType::Codex) => {
                "Archive this Codex thread?".into()
            }
    ...
    pub(in crate::ui::modals::dialogs) fn thread_action_confirm_hint(
        locale: Locale,
    ) -> &'static str {
    ...
    pub(in crate::ui::modals::dialogs) fn thread_action_cancel_hint(
        locale: Locale,
    ) -> &'static str {
        if is_cjk_locale(locale) {
            "任意键取消"
...
mod meta {
    ...
    pub(in crate::ui::modals::dialogs) fn thread_meta_editor_help_text(
        locale: Locale,
    ) -> &'static str {
        if is_cjk_locale(locale) {
