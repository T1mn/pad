...
mod hook {
    ...
    pub fn record_hook_event(
        ...
        let Some(session_id) = clean_text(event.session_id.as_deref().or(fallback_session_id))
        else {
            return;
        };
...
pub use cache::record_cache_write;
pub use hook::record_hook_event;
pub use preview::record_preview_assessment;
