...
- `security.rs`：provider 配置、认证和备份文件的 owner-only 权限测试。
...
- `opencode.rs` / `opencode_runtime.rs` / `opencode_safety.rs`：OpenCode JSON/JSONC provider、model、managed state、runtime overlay 与损坏配置保护测试。
