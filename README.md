...
| `E` / `S` / `I` / `O` / `P` / `Y` | Export/import OpenCode JSON, stats, diagnostics, or attach to a server URL from clipboard |
| `r` | Refresh |
...
- 🟠 OpenCode (`opencode`): launcher and pane attach, relay/model config, SQLite history, session preview, usage/share metadata, archive/unarchive, `opencode export` / `--sanitize`, `opencode import`, project `opencode stats`, provider/model diagnostics, `opencode attach`, and `opencode --session` resume
