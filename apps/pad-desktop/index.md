...
- `electron/main/`：单实例 macOS 窗口、CSP 和 PAD Rust sidecar 的唯一进程所有者；renderer 请求前强制 hello 协商 protocol v2，拒绝把旧 v1 私有记录透传给 Electron。
- `electron/preload/`：`contextIsolation` 下的 bootstrap/request/subscribe 白名单桥。
- `shared/protocol/`：renderer、preload 与 main 共用的 Desktop protocol v2 类型；包含统一 1 MiB 帧上限、安全记录、Rust-owned auth/terminal/UI-state DTO、server event 与本地菜单事件契约。
